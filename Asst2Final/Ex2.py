...
# Requirement 2: Implement a new non-trivial sales analytic (simple variations of existing analytics would not satisfy this requirement).
...
def top_selling_products_by_region(sales_data):
    pivot_table = pd.pivot_table(sales_data, index='sales_region', columns='product_category', values='sales', aggfunc='sum')
    print("\nTop-Selling Products by Region:")
    print(pivot_table)
    prompt_export_to_csv(pivot_table, "top_selling_products_by_region")
    return pivot_table
...
# Create a new menu option to generate a new non-trivial sales analytic
def display_menu(sales_data):
    menu_options = (
        ...
        ("Show top-selling products by region", top_selling_products_by_region), # New non-trivial analytic
